(()=>{
  ...
  let portalOpen=false,slide=0,track=0,audioContext=null,analyser=null,sourceNode=null,animationFrame=0,quoteTimer=0,sidBusy=false,returnFocus=null,touchStart=0,fadeTimer=0;
  ...
  document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!extras.classList.contains('hidden'))closeExtras();else if(event.key==='Escape'&&portalOpen)closeLightbox()});
  ...
  function portalMarkup(){
    ...
    return `<div class="archive-intro" aria-live="polite" aria-atomic="true" hidden><p>ACCESSING ROYAL ARCHIVE…</p></div><div class="royal-atmosphere" aria-hidden="true">${symbols}</div><div class="royal-quote-field">${quotes}</div>
      ...
      <section class="archive-section"><h2 class="archive-heading">THE ROYAL ARCHIVES</h2><p class="archive-lede">Two moments. Both permanently entered into the royal record. Tap either portrait to inspect it at full scale.</p><div class="archive-duo"><figure class="archive-image-frame" data-enlarge-src="assets/sid-federer-handshake.png" data-enlarge-alt="Sid Basa shaking hands with Roger Federer in the royal hall." tabindex="0" role="button" aria-label="Enlarge The Handshake"><img src="assets/sid-federer-handshake.png" loading="lazy" width="1672" height="941" alt="Sid Basa shaking hands with Roger Federer in the royal hall."><figcaption>I. The Handshake <span>Open full screen ↗</span></figcaption></figure><figure class="archive-image-frame offset" data-enlarge-src="assets/sid-federer-kneeling.png" data-enlarge-alt="Sid Basa holding his racket while Federer kneels in the royal hall." tabindex="0" role="button" aria-label="Enlarge The Recognition"><img src="assets/sid-federer-kneeling.png" loading="lazy" width="1672" height="941" alt="Sid Basa holding his racket while Federer kneels in the royal hall."><figcaption>II. The Recognition <span>Open full screen ↗</span></figcaption></figure></div></section>
      <section class="archive-section" aria-labelledby="winsHeading"><h2 id="winsHeading" class="archive-heading">GREATEST WINS</h2><p class="archive-lede">A complete Slam sweep, preserved in full colour. Each image opens into the hall-sized version.</p><div class="season-line" aria-label="2027 Grand Slam timeline"><button class="ao" type="button" data-slide-jump="1">Melbourne</button><button class="rg" type="button" data-slide-jump="2">Paris</button><button class="wb" type="button" data-slide-jump="3">London</button><button class="us" type="button" data-slide-jump="4">New York</button></div><div class="wins-carousel" data-carousel tabindex="0" aria-roledescription="carousel" aria-label="Greatest wins"><div class="wins-track">${wins.map((win,index)=>`<article class="win-slide${index===0?' active':''}" data-win-slide="${index}" aria-hidden="${index===0?'false':'true'}"><img data-src="${win.src}" data-enlarge-src="${win.src}" data-enlarge-alt="${win.alt}" width="1672" height="941" alt="${win.alt}" role="button" tabindex="${index===0?'0':'-1'}" aria-label="Enlarge ${win.name}"><div class="win-caption"><small>${win.event}</small><b>${win.name}</b><span>${win.score} · Open full screen ↗</span></div></article>`).join('')}</div><div class="carousel-controls"><button class="carousel-arrow" type="button" data-slide-prev aria-label="Previous win">‹</button><div class="carousel-dots">${wins.map((_,index)=>`<button class="carousel-dot" type="button" data-slide-jump="${index}" aria-label="Show win ${index+1}" aria-current="${index===0?'true':'false'}"></button>`).join('')}</div><button class="carousel-arrow" type="button" data-slide-next aria-label="Next win">›</button></div></div></section>
      <section class="archive-section"><h2 class="archive-heading">THE FOUR CROWNS</h2><p class="archive-lede">Not clip-art, not borrowed glory: four proper trophies for the season that stopped behaving normally.</p><div class="crown-cabinet"><article class="crown-item">${trophySvg('Australian Open 2027','ao')}<small>AO 2027</small><b>Sinner</b><span>6–2  6–0  7–6(4)</span></article><article class="crown-item">${trophySvg('Roland-Garros 2027','rg')}<small>RG 2027</small><b>Alcaraz</b><span>4–6  6–2  6–3  6–1</span></article><article class="crown-item">${trophySvg('Wimbledon 2027','wb')}<small>Wimbledon 2027</small><b>Sinner</b><span>6–2  6–4  6–4</span></article><article class="crown-item">${trophySvg('US Open 2027','us')}<small>US Open 2027</small><b>Djokovic</b><span>2–6  6–7(6)  6–3  6–0  6–1</span></article></div></section>
      <section class="archive-section"><h2 class="archive-heading">THE NUMBERS, ENCHANTED</h2><p class="archive-lede">The record book attempted neutrality. It was overruled by the evidence.</p><div class="records-layout"><div class="record-strip"><div class="record-stat"><span>2027 SLAMS</span><b>4</b></div><div class="record-stat"><span>SLAM FINALS</span><b>4–0</b></div><div class="record-stat"><span>FINAL SETS LOST</span><b>3</b></div><div class="record-stat"><span>FEDERER HANDSHAKES</span><b>1</b></div><div class="record-stat"><span>AURA</span><b>∞</b></div></div><aside class="h2h"><h3>HEAD-TO-HEAD</h3><ul><li><span>Jannik Sinner</span><b>2–0</b></li><li><span>Carlos Alcaraz</span><b>1–0</b></li><li><span>Novak Djokovic</span><b>1–0</b></li><li class="jayden"><span>Jayden Bone</span><small>6–0  6–0</small><b>1–0</b></li></ul><p>The numbers are illuminated, the testimony is unanimous.</p></aside></div></section>
      ...
      <section class="archive-plaque"><p>Every legend needs a closing plaque. This one has a kingdom behind it.</p><img src="assets/sid-honour-plaque-transparent.png" loading="lazy" width="2048" height="682" alt="In Honour of Siddharth R. Basa. Champion. Monarch. Wielder of the Blade. The Royal Tennis Archive · Est. 2027."></section>
      <div class="archive-lightbox hidden" data-lightbox role="dialog" aria-modal="true" aria-label="Enlarged archive image"><button type="button" data-lightbox-close aria-label="Close image">×</button><img data-lightbox-image alt=""><p>Tap outside the image or press Escape to return to the archive.</p></div>
  ...
  function renderPortal(){
    ...
    archive.querySelectorAll('[data-enlarge-src]').forEach(item=>{item.addEventListener('click',()=>openLightbox(item.dataset.enlargeSrc,item.dataset.enlargeAlt));item.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openLightbox(item.dataset.enlargeSrc,item.dataset.enlargeAlt)}})});
  ...
  function openLightbox(src,alt){const box=archive.querySelector('[data-lightbox]'),image=archive.querySelector('[data-lightbox-image]');if(!box||!image)return;image.src=src;image.alt=alt||'Enlarged archive image';box.classList.remove('hidden');document.body.classList.add('archive-lightbox-open');box.querySelector('[data-lightbox-close]').focus()}
  function closeLightbox(){const box=archive.querySelector('[data-lightbox]');if(!box)return;box.classList.add('hidden');document.body.classList.remove('archive-lightbox-open')}
  ...
  function sidEffect(){
    if(sidBusy||!portalOpen)return;sidBusy=true;const restoreVolume=music.volume,wasPlaying=!music.paused;
    fadeMusic(0,320,()=>{if(wasPlaying)music.pause();setTimeout(()=>{
      gong.currentTime=0;gong.muted=false;gong.volume=1;gong.play().catch(()=>{});
      const stage=document.createElement('div');stage.className='sid-rise-stage';stage.setAttribute('aria-hidden','true');
      for(let i=0;i<170;i++){const piece=document.createElement('i');piece.className='royal-confetti';piece.style.setProperty('--x',`${Math.round((Math.random()-.5)*Math.max(innerWidth*1.25,760))}px`);piece.style.setProperty('--y',`${Math.round(-110-Math.random()*Math.max(innerHeight*.9,420))}px`);piece.style.setProperty('--r',`${Math.round((Math.random()-.5)*1280)}deg`);piece.style.setProperty('--delay',`${Math.random()*.24}s`);piece.style.setProperty('--confetti-color',['#fff6cc','#e9bc50','#b67928','#4d8eff','#e8555d'][i%5]);stage.appendChild(piece)}
      const figure=document.createElement('img');figure.className='sid-rise-figure';figure.src='assets/sid-royal-rise-transparent.png';figure.alt='';stage.appendChild(figure);document.body.appendChild(stage);if(!reduced.matches)document.body.classList.add('royal-shake');setTimeout(()=>document.body.classList.remove('royal-shake'),720);setTimeout(()=>{stage.remove();sidBusy=false;if(wasPlaying){music.volume=0;playMusic();fadeMusic(restoreVolume,1800)}},reduced.matches?1000:5200);
    },720)});
  }
