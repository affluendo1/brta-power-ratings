...
function resultDetailHTML(match){
  ...
  let rows=(match.rubbers||[]).map(r=>{const home=r.type==='Doubles'?pairNames(r.home):resultPerson(r.home),away=r.type==='Doubles'?pairNames(r.away):resultPerson(r.away);return'<div class="rubber-row"><small class="rubber-code" title="'+esc(r.type)+' · '+esc(r.position)+'">'+rubberCode(r)+'</small><b class="rubber-home '+(r.winner===r.home?'winner':'')+'" title="'+esc(r.home)+'">'+home+'</b><strong>'+esc(r.score)+'</strong><b class="rubber-away '+(r.winner===r.away?'winner':'')+'" title="'+esc(r.away)+'">'+away+'</b></div>'}).join('');
  return head+(rows?'<div class="rubber-list">'+rows+'</div>':'<div class="notice">TROLS has not published an individual scorecard for this match.</div>')
}
function openMatch(id){const match=findMatch(id);if(match){selectedMatch=id;switchPage('results');$('#resultContent').innerHTML=resultDetailHTML(match);$('#resultOverlay').classList.remove('hidden');document.body.classList.add('modal-open')}}
function renderFixtures(){
  ...
  $('#fixturesList').innerHTML=rounds.length?rounds.map(r=>'<section class="fixture-round"><div class="fixture-round-head"><h2>Round '+r.round+'</h2><span>'+esc(r.date)+'</span></div><div class="fixture-list">'+r.fixtures.map(f=>{
    ...
    return'<button class="fixture-match '+(complete?'complete':'')+'" data-match-id="'+esc(f.fixtureId)+'"><span>'+esc(f.home)+'</span><b>'+esc(middle)+'</b><span>'+esc(f.away)+'</span><small>'+esc(sub)+'</small></button>'
...
document.addEventListener('click',e=>{
  ...
  const jump=e.target.closest('[data-section-jump]');if(jump){const code=jump.dataset.sectionJump,meta=DATA.catalog.find(x=>x.section_code===code);renderChoice('competitionSelect',[...new Map(DATA.catalog.map(x=>[x.competition_code,x.competition_label.replace(' - ',' · ')])).entries()],meta.competition_code);fillSections(meta.competition_code,code);loadSection(code);ratingView='singles';$$('.subtab').forEach(x=>x.classList.toggle('active',x.dataset.rating==='singles'));return}
  ...
  if(e.target.dataset.close)close('#profileOverlay');if(e.target.dataset.roundClose)close('#roundOverlay');if(e.target.dataset.settingsClose)close('#settingsOverlay');if(e.target.dataset.resultClose)close('#resultOverlay');
});
...
applyProfilePosition();setupSelectors();showSyncToast();loadSection(choiceValue('sectionSelect'));
