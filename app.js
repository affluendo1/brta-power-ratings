...
let theme=localStorage.getItem('brta-theme')||'auto',interfaceMode=localStorage.getItem('brta-interface')||'future',accent=localStorage.getItem('brta-accent')||'green',profilePosition=localStorage.getItem('brta-profile-position')||'right';
...
function setupSelectors(){
  ...
  renderChoice('competitionSelect',comps.map(([c,l])=>[c,l.replace(' - ',' · ')]),meta.competition_code);
...
async function loadSection(code){
  ...
  try{
    const payload=await fetch('data/site/sections/'+code+'.json?v='+encodeURIComponent(DATA.globalSync.checkedAt||''),{cache:'no-store'}).then(r=>{if(!r.ok)throw Error('Section data could not be loaded');return r.json()});
    ...
    renderChoice('competitionSelect',[...new Map(DATA.catalog.map(x=>[x.competition_code,x.competition_label.replace(' - ',' · ')])).entries()],D.meta.competition_code);fillSections(D.meta.competition_code,code);
...
function renderGlobalRatings(){
  ...
  for(const x of rows){
    if(ratingView==='overall'){
      ...
      out+='<tr class="'+(x.qualified?'':'provisional')+'"><td>'+globalPersonButton(x)+'</td><td>'+esc(x.team)+'</td><td>'+sectionCell(x)+'</td><td><b>'+x.rating+'</b></td><td>'+s+'</td><td>'+d+'</td><td>'+(x.qualified?'Ranked':'Provisional')+'</td><td>±'+(x.se??'—')+'</td></tr>';
    }else{
      ...
      out+='<tr class="'+(x.qualified?'':'provisional')+'"><td>'+globalPersonButton(x)+'</td><td>'+esc(x.team)+'</td><td>'+sectionCell(x)+'</td><td><b>'+x.rating+'</b></td><td>'+x.wins+'–'+x.losses+'</td><td>'+x.gf+'–'+x.ga+'</td><td>'+evidence+'</td><td>±'+(x.se??'—')+'</td></tr>';
    }
  }
...
function renderRatings(){
  ...
  for(const x of rows){const qual=ratingQualified(x,min,type);if(qual)rank++;if(!qual&&!showProv)continue;if(q&&!x.player.toLowerCase().includes(q))continue;if(tf&&x.team!==tf)continue;const cls=qual?band.classFor(x.rating):'provisional';if(qual&&cls!==lastBand){out+=bandDivider(cls,band,type==='pair'?9:8);lastBand=cls}
    ...
    if(type==='pair'){const pair=x.player.split(' / ').map(personButton).join('<span class="pair-sep"> / </span>'),effect=(x.pairEffect>=0?'+':'')+(x.pairEffect??0);out+='<tr class="'+cls+'"><td>'+(qual?rank:'—')+'</td><td>'+pair+'</td><td>'+esc(x.team)+'</td><td><b>'+x.rating+'</b></td><td>'+(x.individualAverage??'—')+'</td><td class="'+((x.pairEffect??0)>=0?'performance-above':'performance-below')+'">'+effect+'</td><td>'+x.wins+'–'+x.losses+'</td><td>'+x.matches+'</td><td>±'+x.se+'</td></tr>'}
...
function matchCard(match){
  ...
  return'<button class="result-match" data-match-id="'+esc(match.fixtureId)+'"><span class="result-team '+(homeWon?'won':'')+'"><b>'+esc(match.home)+'</b><small>'+homeMeta+'</small></span><span class="result-score"><b>'+esc(central)+'</b><small>'+ (complete?'Rubbers':scored?'Team points':'')+'</small></span><span class="result-team '+(awayWon?'won':'')+'"><b>'+esc(match.away)+'</b><small>'+awayMeta+'</small></span></button>'
}
function renderResults(){const rounds=D.results||[];if(!rounds.length){$('#resultsList').innerHTML='<div class="notice">No published results yet.</div>';return}$('#resultsList').innerHTML=rounds.map(round=>'<section class="result-round"><header class="result-round-head"><h2>Round '+round.round+'</h2><span>'+esc(round.date)+'</span></header><div class="result-round-matches">'+round.fixtures.map(matchCard).join('')+'</div></section>').join('')}
...
function resultDetailHTML(match,contextPlayer=null){
  ...
  let head='<div class="result-detail-head"><span>Round '+match.round+' · '+esc(match.date)+'</span><h2>'+esc(match.home)+' <b>'+result+'</b> '+esc(match.away)+'</h2><p>'+outcome+(complete?'':' · '+esc(match.status))+'</p>'+metrics+'</div>';
...
function renderFixtures(){
  ...
  $('#fixturesList').innerHTML=rounds.length?rounds.map(r=>'<section class="fixture-round"><div class="fixture-round-head"><h2>Round '+r.round+'</h2><span>'+esc(r.date)+'</span></div><div class="fixture-list">'+r.fixtures.map(f=>{
...
function predictionHTML(match){
  ...
  const summary=!ready?'<div class="notice prediction-not-ready"><b>Complete both lineups to calculate the fixture.</b><span>Select exactly '+needed+' named players for each team. No missing player is silently replaced with a 1500 rating.</span></div>':('<div class="prediction-summary"><div><span>'+esc(match.home)+' win</span><b>'+Math.round(100*prediction.homeWin)+'%</b></div><div><span>'+esc(match.away)+' win</span><b>'+Math.round(100*prediction.awayWin)+'%</b></div><div><span>Exact draw</span><b>'+Math.round(100*prediction.draw)+'%</b></div><div><span>Expected rubbers</span><b>'+prediction.expectedRubbers.toFixed(1)+'–'+(prediction.rubbers.length-prediction.expectedRubbers).toFixed(1)+'</b></div></div>');
  ...
  return'<div class="overlay-title"><div class="kicker">FIXTURE PREDICTION</div><h2>'+esc(match.home)+' vs '+esc(match.away)+'</h2><p>Round '+esc(match.round||'')+' · Select the available lineup and doubles pairings.</p></div>'+summary+'<div class="lineup-pickers">'+picker(match.home,state.home,'home')+picker(match.away,state.away,'away')+'</div><section class="prediction-section"><h3>Rubber projections</h3><p>'+formatNote+' For Sets fixtures, 3–3 outcomes are decided from the simulated game margins instead of being handed to the pre-match favourite.</p><div class="prediction-rubbers">'+rows+'</div></section>';
}
...
function openHistoricalTable(round){const rows=historicalRows(round),date=historicalRoundDate(round);$('#roundContent').innerHTML='<div class="overlay-title"><div class="kicker">ROUND '+round+' RATINGS</div><h2>After Round '+round+'</h2><p>'+esc(date)+' · '+esc(D.meta.section_label)+'</p></div><div class="table-wrap"><table><thead><tr><th>#</th><th>Player</th><th>Team</th><th>Power</th><th>Movement</th><th>Matches</th><th>Uncertainty</th></tr></thead><tbody>'+rows.map(row=>'<tr class="'+(row.rank?'':'provisional')+'"><td>'+(row.rank??'—')+'</td><td><button class="person" data-history-player="'+esc(row.player)+'" data-history-round="'+round+'">'+esc(row.player)+'</button></td><td>'+esc(row.team)+'</td><td><b>'+row.rating+'</b></td><td>'+(row.change==null?'—':(row.change>=0?'+':'')+row.change)+'</td><td>'+row.matches+'</td><td>±'+row.se+'</td></tr>').join('')+'</tbody></table></div>';$('#roundOverlay').classList.remove('hidden');document.body.classList.add('modal-open')}
...
function renderAnalytics(){
  ...
  $('#sectionLeaderCard').innerHTML=leader?'<div class="analytics-card-head"><div><span class="kicker">SECTION LEADER</span><h2>'+personButton(leader.player)+'</h2><p>'+esc(leader.team)+' · '+leader.rating+' Power</p></div><b class="leader-share">'+leader.expectedGameShare+'%</b></div><div class="analytics-mini-grid"><div><span>Record</span><b>'+leader.wins+'–'+leader.losses+'</b></div><div><span>Gap to #2</span><b>+'+leader.runnerUpGap+'</b></div><div><span>Vs 1500 game share</span><b>'+leader.expectedGameShare+'%</b></div></div>':'<div class="notice">No qualified section leader yet.</div>';
  ...
  }).join('')||'<tr><td colspan="8">No qualified expectation rows yet.</td></tr>';
  $('#leadersBody').innerHTML=(DATA.leaders||[]).map(row=>'<tr><td>#'+row.dominanceRank+'</td><td>'+globalPersonButton({player:row.player,sectionCode:row.sectionCode})+'</td><td><button class="section-jump-link" data-section-jump="'+esc(row.sectionCode)+'">'+esc(row.sectionLabel)+'</button><small class="section-result-comp">'+esc(row.competitionLabel.replace(' - ',' · '))+'</small></td><td><b>'+row.rating+'</b></td><td>'+row.expectedGameShare+'%</td><td>+'+row.runnerUpGap+'</td></tr>').join('');
}
...
function renderAll(){statusStrip();teamOptions();renderRatings();renderTeams();renderStandings();renderResults();renderFixtures();renderAnalytics();$('#roundBtn').textContent=D.roundOverview?'Round '+D.roundOverview.round+' overview':'Latest round'}
...
applyProfilePosition();setupSelectors();$('#searchScope').value=ratingScope;showSyncToast();loadSection(choiceValue('sectionSelect'));
