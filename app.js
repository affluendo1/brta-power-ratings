...
function decorateTeamNames(root=document.body){
  const names=[...new Set([...(teams||[]).map(x=>x.team),...(D?.standings||[]).map(x=>x.team),...(D?.upcomingFixtures||[]).flatMap(round=>round.fixtures.flatMap(f=>[f.home,f.away]))].filter(Boolean))].sort((a,b)=>b.length-a.length);
  if(!root||!names.length)return;
  const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT),nodes=[];
  while(walker.nextNode())nodes.push(walker.currentNode);
  for(const node of nodes){
    if(node.parentElement?.closest('.team-colour,script,style'))continue;
    const text=node.nodeValue;if(!names.some(name=>text.includes(name)))continue;
    const fragment=document.createDocumentFragment();let rest=text;
    while(rest){const name=names.find(item=>rest.includes(item));if(!name){fragment.append(rest);break}const index=rest.indexOf(name);if(index)fragment.append(rest.slice(0,index));const holder=document.createElement('span');holder.innerHTML=teamName(name);fragment.append(...holder.childNodes);rest=rest.slice(index+name.length)}
...
function setupSelectors(){
  const comps=[...new Map(viewCatalog.map(x=>[x.competition_code,x.competition_label])).entries()];
...
function renderAll(){statusStrip();teamOptions();renderRatings();renderTeams();renderStandings();renderResults();renderFixtures();renderAnalytics();decorateTeamNames();$('#roundBtn').textContent=D.roundOverview?(D.roundOverview.label||('Round '+D.roundOverview.round))+' overview':'Latest round'}
...
new MutationObserver(records=>{for(const record of records)for(const node of record.addedNodes)if(node.nodeType===Node.ELEMENT_NODE||node.nodeType===Node.TEXT_NODE)decorateTeamNames(node.nodeType===Node.TEXT_NODE?node.parentElement:node)}).observe(document.body,{childList:true,subtree:true});
